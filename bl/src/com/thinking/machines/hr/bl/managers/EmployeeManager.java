...
public int countOfEmployeeWithSameDesignation(int designationCode) throws BLException
{
BLException blException=new BLException();
if(designationCode<=0) blException.addException("designationCode","Invalid Designation Code");
if(blException.hasException("designationCode")) throw blException;
List<EmployeeInterface> employees;
employees=designationWiseEmployees.get(designationCode);
if(employees==null) throw new BLException("DesignationCode not exists against any employees");
if(blException.hasException("designationCode")) throw blException;
...
if(vPanNumber==null || vPanNumber.trim().length()==0) blException.addException("PanNumber","Required panNumber");
...
if(panNumberWiseMap.containsKey(vPanNumber)) blException.addException("panNumber","Pan Number already exist");
if(blException.hasException("designationCode") || blException.hasException("aadharCardNumber") || blException.hasException("panNumber") || blException.hasException("employeeId") || blException.hasException("employeeName"))
{
throw blException;
}
...
{
Comparator<EmployeeInterface> employeeIdComparator=new Comparator<EmployeeInterface>(){
public int compare(EmployeeInterface left,EmployeeInterface right)
{
...
{
Comparator<EmployeeInterface> nameComparator=new Comparator<EmployeeInterface>(){
public int compare(EmployeeInterface left,EmployeeInterface right)
{
...
EmployeeInterface tmp;
tmp=employeeIdWiseOrderedList.get(0);
if(nameComparator.compare(dsEmployee,tmp)>=0)
{
...
if(employeeWiseMap.containsKey(vEmployeeId)==false) blException.addException("employee","Invalid EmployeeId");
String vName=employee.getName();
if(vName==null || vName.trim().length()==0) blException.addException("employeeName","Required employee's name");
vName=vName.trim();
DesignationInterface vDesignation=employee.getDesignation();
int vDesignationCode=vDesignation.getCode();
//boolean designationCodeExists=designationWiseEmployee.containsKey(vDesignationCode);
DesignationManager designationManager=DesignationManager.getDesignationManager();
boolean designationCodeExists=designationManager.codeExists(vDesignationCode);
if(designationCodeExists==false) blException.addException("designationCode","Invalid Designation Code");
Date vDateOfBirth=employee.getDateOfBirth();
...
if(panNumberWiseMap.containsKey(vPanNumber) && panNumberWiseMap.get(vPanNumber).getEmployeeId().equals(vEmployeeId)==false)
{
blException.addException("panNumber","Pan Number already exist");
}
if(blException.hasException("designationCode") || blException.hasException("aadharCardNumber") || blException.hasException("panNumber") || blException.hasException("employeeId") || blException.hasException("employeeName"))
{
throw blException;
